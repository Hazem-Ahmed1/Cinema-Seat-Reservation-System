...
type RegisterForm() as this =
    ...
    let createTextBox label yPos isPassword =
        let labelControl = new Label(Text = label, Location = Point(50, yPos), AutoSize = true)
        let textBox = 
            if isPassword then
    ...
    do
        ...
        let confirmPasswordLabel, confirmPasswordTextBox = createTextBox "Confirm Password" 260 true
        ...
        this.Controls.AddRange([| 
            ...
            registerButton; loginLinkLabel 
        |])

        // Register button click event
        registerButton.Click.Add(fun _ ->
            if passwordTextBox.Text <> confirmPasswordTextBox.Text then
                MessageBox.Show("Passwords do not match", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error) |> ignore
            else
                ...
                | Ok _ -> 
                    ...
                    this.Close()
                | Error msg -> 
                    MessageBox.Show(msg, "Registration Failed", MessageBoxButtons.OK, MessageBoxIcon.Error) |> ignore
        )
