...
module UserAuthentication =
    ...
    let private userDatabasePath = @"D:\projects\Cinema-Seat-Reservation-System\Database\Users.txt"
    
    let registerUser (username: string) (password: string) (email: string) =
        if String.IsNullOrWhiteSpace(username) || String.IsNullOrWhiteSpace(password) || String.IsNullOrWhiteSpace(email) then
            ...
            try
                ...
                if File.Exists(userDatabasePath) then
                    ...
                    if existingUsers |> Array.exists (fun line -> 
                        ...
                        let userEntry = sprintf "%s,%s,%s,%s" newUser.Username newUser.PasswordHash newUser.Email (newUser.CreatedAt.ToString("o"))
                    ...
                    Ok newUser
            with 
            | ex -> Error (sprintf "Registration failed: %s" ex.Message)
    ...
    let authenticateUser (username: string) (password: string) =
        ...
        with
        | ex -> Error (sprintf "Authentication failed: %s" ex.Message)